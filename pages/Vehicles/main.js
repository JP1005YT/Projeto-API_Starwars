...
async function pegarInfos(link) {
  ...
  function ConstruirTodos(array){
    array.results.forEach(function(element,n){
        ...
        img.setAttribute("src",`http://localhost/starWars2.0/resources/vehicles/${element.name.toLowerCase()}.jpg`)
        ...
        div.addEventListener("click",e =>{
            window.location.href = `http://localhost/starWars2.0/pages/vehicles?id=${ultimoDigito}`
        })
...
async function ConstruirCada(array){
    ...
    div_o.querySelector("img").setAttribute("src",`http://localhost/starWars2.0/resources/vehicles/${array.name.toLowerCase()}.jpg?c=${Math.random()}`)
